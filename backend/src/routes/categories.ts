...
export function createCategoriesRouter(db: any): Router {
  ...
  router.put('/:id', (req: Request, res: Response, next: NextFunction) => {
    try {
      const existing = db.select().from(categories).where(sql`id = ${req.params.id}`).get();
      ...
      db.update(categories).set({ name: name.trim() }).where(sql`id = ${req.params.id}`).run();
  ...
  router.delete('/:id', (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      db.delete(categories).where(sql`id = ${req.params.id}`).run();
