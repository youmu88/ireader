...
export default function ReaderPage() {
  ...
  // ── 错误态（含 TXT 暂不支持提示）：错误页也用阅读主题背景，避免状态栏/顶部露出白色 ──
  if (error) {
    return (
      <div
        className="fixed inset-0 flex flex-col items-center justify-center gap-5 px-8"
        style={{ background: themeSpec.background, color: themeSpec.color }}
      >
        <p className="text-center leading-relaxed" style={{ opacity: 0.7 }}>{error}</p>
